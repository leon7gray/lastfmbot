...
def get_topgenres(user):
    # Query db, look for where username is the user and "topgenre" key exists
    current = users.find_one({"username": user, ("topgenres") : {"$exists" : True}})

    if (current):
        return current[("topgenres")]
    else:
        return None
    
def insert_topgenres(user, genres):
    # Update user, set "topgenres" to a dict holding genres and last updated as keys
    current_time = datetime.datetime.now()
    users.update_one({"username": user}, { "$set": { "topgenres": {"genres": genres, "lastupdated": current_time} } }, upsert=True)
