...
def insert_topartists(user, artists, time):
    current_time = datetime.datetime.now()
    users.update_one({"username": user}, { "$set": { ("topartist" + time): {"artists": artists, "lastupdated": current_time} } })

def get_topartists(user, time):
    current = users.find_one({"username": user, ("topartist" + time) : {"$exists" : True}})
    
    if (current != None):
        return current[("topartist" + time)]
    else:
        return None
