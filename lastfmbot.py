...
def get_usertopgenres(user):
    url = "?method=user.gettoptags&user=" + user + "&api_key=" + API_KEY + "&format=json&limit=5"
    result = requests.get(ROOT_URL + url)
    return(result.json()["toptags"]["tag"])
...
# Displays the top 5 genres the user listens to
@bot.command()
async def topgenres(ctx):
    # Query database for the discord user and returns their default_user
    user = database.get_default_user(ctx.author.name)

    # User not found -> user never used bot to set account
    if (user == None):
        await ctx.send("Please set your last.fm username by using $set (username)")
        return
    
    # Check to see if db has info on user's top genres
    result = database.get_topgenres(ctx.author.name)

    if (result == None):
        # No info in db, use lastfm api and insert info into db
        database.insert_topgenres(ctx.author.name, get_usertopgenres(user))
        result = database.get_topgenres(ctx.author.name)
    
    message = [(tag["name"] + " - " + tag["count"] + " times") for tag in result['genres']]
    bot_message = "```Overall, your top 5 genres are\n"
    for i in range(len(message)):
        message[i] = str(i + 1) + ". " + message[i] + '\n'
        bot_message += message[i]
    bot_message += "last updated: " + result["lastupdated"].strftime("%Y-%m-%d %H:%M:%S") + "```"
